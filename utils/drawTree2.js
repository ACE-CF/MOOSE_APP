...
export function drawTree2(data, question,containerId, setToastMessage, setShowToast, setPopupContent, setPopupPosition) {
...
//     treeData.name = question;
// //from here
//     const wrapTextWithEllipsis = (text, maxCharsPerLine = 40, maxLines = 3) => {
//     const lines = [];
//     let current = 0;

//     for (let i = 0; i < maxLines; i++) {
//         let line = text.slice(current, current + maxCharsPerLine);
//         current += maxCharsPerLine;

//         // 如果已经到最后一行，但还有剩余文本，加省略号
//         if (i === maxLines - 1 && current < text.length) {
//             line = line.slice(0, maxCharsPerLine - 1) + '…';
//         }
...
//     const hierarchyData = d3.hierarchy(treeData);
//     const nodeWidth = 600, nodeHeight = 150, nodeSpacing = 20;
//     // const treeLayout = d3.tree().nodeSize([nodeWidth + nodeSpacing, nodeHeight + nodeSpacing]);
//     const treeLayout = d3.tree().nodeSize([nodeWidth + nodeSpacing, nodeHeight + 100]);
//     treeLayout(hierarchyData);
...
//             .attr("height", "100%") // ✅ 设置为百分比，铺满容器高
//             // .attr("viewBox", `0 0 ${svgWidth} ${svgHeight}`) // ✅ 自适应内容尺寸
//             // .attr("viewBox", `${minX - margin} 0 ${svgWidth} ${svgHeight}`)  
...
//     //     .style("fill", "#fff")
//     //     // .text(d => truncateText(d.data.name, 15));
//     //     .text(d => truncateText(d.data.name, 40));
//     const lineHeight = 35;

//     nodes.append("text")
//         .attr("text-anchor", "middle")
//         .each(function(d) {
//             const lines = wrapTextWithEllipsis(d.data.name, 40, 3);
//             const totalHeight = lines.length * lineHeight;
//             const startY = -totalHeight / 2 + lineHeight / 2+10; // 居中显示
